...
Pre prenos na iný Mac spustite `GMF_CREATE_DMG=1 scripts/build-macos-app.sh`. V `dist/` vznikne `.dmg` s názvom, napríklad `Give_Me_Five_Editor-3.13.3-arm64.dmg`. Ak macOS nepovolí vytvorenie obrazu disku, skript automaticky vytvorí Finder-kompatibilný `.zip` s rovnakou aplikáciou.
